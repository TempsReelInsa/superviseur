...
#define DBG_STATUS (1<<7)
#define DBG_MISSION (1<<8)
#define DBG_ALL (DBG_WATCHDOG | DBG_SEND_MONITOR | DBG_CONNECT_ROBOT | DBG_RECV_MONITOR | DBG_MOVE_ROBOT | DBG_BATTERY_STATE | DBG_IMAGE | DBG_STATUS | DBG_MISSION)

#define DEBUG_WHAT DBG_ALL & ~DBG_SEND_MONITOR
...
#if DEBUG_WHAT & DBG_STATUS
	#define LOG_STATUS(fmt, ...) DPRINTF_C(BLINK_RED_WHITE, fmt, ## __VA_ARGS__)
#else
	#define LOG_STATUS(fmt, ...)
#endif

#if DEBUG_WHAT & DBG_MISSION
	#define LOG_MISSION(fmt, ...) DPRINTF_C(CYAN, fmt, ## __VA_ARGS__)
#else
	#define LOG_MISSION(fmt, ...)
#endif
